...
The API lives in `backend/` and uses raw SQL against PostgreSQL (Supabase-compatible).
...
```
Edit `.env` with your Supabase connection string values (`DB_HOST`, `DB_USER`, `DB_PASSWORD`, etc.).

4. Apply the database schema in Supabase SQL Editor (or local Postgres):
   - Run `database/schema_postgresql.sql`
   - Optionally run `database/seed.sql` (creates `admin@example.com` / `adminpass`)

5. Start the API server:
```bash
python run_api.py
# or: python manage.py runserver  (requires DB reachable at startup)
```

API docs: `http://localhost:8000/api/docs` · OpenAPI: `http://localhost:8000/api/openapi.json`

### Automated API / database tests

From the project root (starts servers, runs tests, stops servers):

```bash
run-tests.bat
# or: python testing/run_with_servers.py
```

See `testing/README.md` for Selenium and manual options.

### Frontend ↔ API

1. Copy root `.env.example` to `.env` and set:
```
VITE_API_URL=http://localhost:8000/api
```

2. The HTTP client is at `src/app/services/api.ts`. Wire `App.tsx` handlers (`handleLogin`, `handleSignup`, resource loaders) to `auth`, `resources`, `user`, and `admin` exports instead of mock state when you are ready.
