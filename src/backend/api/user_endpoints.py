...
@router.post('/calendar', auth=auth, response={201: CalendarEventOut, 400: dict})
def create_event(request, body: CreateEventSchema):
    uid = request.auth['sub']
    eid = new_id()
    with get_cursor() as cur:
        cur.execute(
...
@router.delete('/calendar/{event_id}', auth=auth, response={200: MessageOut, 404: dict})
